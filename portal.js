(function () {
  ...
  if (dashGrid) {
    ...
    function renderAdminDashboard() {
      var subnavHtml =
        '<div class="dash-card dash-card-wide" style="padding:0; border:none; background:transparent;">' +
        ...
        '</div>';
      dashGrid.innerHTML = subnavHtml;
      dashGrid.querySelectorAll('[data-admin-view]').forEach(function (btn) {
        btn.classList.toggle('is-active', btn.getAttribute('data-admin-view') === adminCurrentView);
        btn.addEventListener('click', function () {
          adminCurrentView = btn.getAttribute('data-admin-view');
          renderAdminDashboard();
        });
      });
